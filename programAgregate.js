...
mongo.connect('mongodb://localhost:27017/learnyoumongo',function(err,db){
  ...
  /*In this exercise, we need to calculate the average price for all documents
    in the prices collection in the database named learnyoumongo lear that have
    the size that will be passed as the first argument to your script.
  */
  //db.collection('prices').find().toArray(function(err,data){
  var match = { $match: { size: size}};
  var avg = { $group: {_id: 'average', average: {$avg: '$price'}}};
  
  db.collection('prices').aggregate([match,avg ]).toArray(function(err,data){
      if(err) throw err;
  
      if(!data.length) throw new Error("No results exception");
